...
#define CAMERA_NAME L"Test Virtual Camera"
...
STDMETHODIMP VCamFactory::CreateInstance(LPUNKNOWN parent,
                                         REFIID,
                                         void** p_ptr) {
  if (!p_ptr) {
    return E_POINTER;
...
static bool RegServer(const CLSID& cls,
                      const wchar_t* desc,
                      const wchar_t* file,
  ...
  RegSetValueExW(subkey, L"ThreadingModel", 0, REG_SZ, (const BYTE*)model,
                 string_size(model));
...
static bool RegServers(bool reg) {
  ...
  if (reg) {
    return RegServer(CLSID_OBS_VirtualVideo, CAMERA_NAME, file);
  } else {
...
static bool RegFilters(bool reg) {
  ...
  if (reg) {
    ...
    hr = fm->RegisterFilter(CLSID_OBS_VirtualVideo, CAMERA_NAME, &moniker,
